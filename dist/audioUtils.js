...
exports.saveDebugAudio = saveDebugAudio;
...
function pcmToWav(pcmBuffer, outputPath, sampleRate = 24000, channels = 1, bitDepth = 16) {
    ...
    // Write to file
    fs_1.default.writeFileSync(outputPath, wavBuffer);
    return outputPath;
}
...
function saveDebugAudio(buffer, type, timestamp = Date.now()) {
    ...
    const debugDir = path_1.default.join(homeDir, 'cheddar', 'debug');
    if (!fs_1.default.existsSync(debugDir)) {
        fs_1.default.mkdirSync(debugDir, { recursive: true });
    }
    const pcmPath = path_1.default.join(debugDir, `${type}_${timestamp}.pcm`);
    const wavPath = path_1.default.join(debugDir, `${type}_${timestamp}.wav`);
    const metaPath = path_1.default.join(debugDir, `${type}_${timestamp}.json`);
    // Save raw PCM
    fs_1.default.writeFileSync(pcmPath, buffer);
