...
interface CheatingDaddyAppElement extends HTMLElement {
    currentView: string;
    layoutMode: string;
...
interface CheddarAPI {
    element(): CheatingDaddyAppElement;
    e(): CheatingDaddyAppElement;
    getCurrentView(): string;
    getLayoutMode(): string;
    setStatus(text: string): void;
    setResponse(response: string): void;
    initializeGemini(profile?: string, language?: string): Promise<void>;
    startCapture(screenshotIntervalSeconds: number | string, imageQuality: string): Promise<void>;
    stopCapture(): void;
    sendTextMessage(text: string): Promise<InvokeResult>;
    handleShortcut(shortcutKey: string): void;
    getAllConversationSessions(): Promise<SessionData[]>;
    getConversationSession(sessionId: string): Promise<SessionData | undefined>;
    initConversationStorage(): Promise<IDBDatabase>;
...
let currentImageQuality: ImageQuality = 'medium'; // Store current image quality for manual screenshots
...
const tokenTracker: TokenTracker = {
    ...
    addTokens(count: number, type: string = 'image'): void {
        const now = Date.now();
        this.tokens.push({
            timestamp: now,
            count: count,
...
ipcRenderer.on('update-status', (event, status: string) => {
...
async function startCapture(screenshotIntervalSeconds: number | string = 5, imageQuality: string = 'medium'): Promise<void> {
    // Store the image quality for manual screenshots
    currentImageQuality = imageQuality as ImageQuality;

    // Reset token tracker when starting new capture session
    tokenTracker.reset();
    console.log('🎯 Token tracker reset for new capture session');

    const audioMode = (localStorage.getItem('audioMode') || 'speaker_only') as AudioMode;
    ...
    try {
        ...
        if (screenshotIntervalSeconds === 'manual' || screenshotIntervalSeconds === 'Manual') {
            ...
            const intervalMilliseconds = parseInt(screenshotIntervalSeconds as string) * 1000;
...
function setupLinuxMicProcessing(micStream: MediaStream): void {
...
function setupLinuxSystemAudioProcessing(): void {
    ...
    audioProcessor.onaudioprocess = async (e: AudioProcessingEvent) => {
        const inputData = e.inputBuffer.getChannelData(0);
        audioBuffer.push(...inputData);
...
function setupWindowsLoopbackProcessing(): void {
    if (!mediaStream) return;
...
async function captureScreenshot(imageQuality: string = 'medium', isManual: boolean = false): Promise<void> {
    ...
    if (!hiddenVideo) {
        ...
        await new Promise<void>(resolve => {
            if (hiddenVideo && hiddenVideo.readyState >= 2) return resolve();
            if (hiddenVideo) {
        ...
        async (blob: Blob | null) => {
            ...
            reader.onloadend = async () => {
                ...
                if (invokeResult.success) {
                    ...
                    console.error('Failed to send image:', invokeResult.error);
                }
            };
...
async function captureManualScreenshot(imageQuality: string | null = null): Promise<void> {
...
function stopCapture(): void {
    ...
    if (isMacOS) {
        ipcRenderer.invoke('stop-macos-audio').catch((err: Error) => {
...
async function sendTextMessage(text: string): Promise<InvokeResult> {
...
async function initConversationStorage(): Promise<IDBDatabase> {
...
async function saveConversationSession(sessionId: string, conversationHistory: any): Promise<IDBValidKey> {
    if (!conversationDB) {
        await initConversationStorage();
    }

    if (!conversationDB) {
        throw new Error('Failed to initialize conversation database');
    }
...
async function getConversationSession(sessionId: string): Promise<SessionData | undefined> {
    if (!conversationDB) {
        await initConversationStorage();
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onerror = () => reject(request.error);
        request.onsuccess = () => resolve(request.result as SessionData | undefined);
    });
}

async function getAllConversationSessions(): Promise<SessionData[]> {
    if (!conversationDB) {
        await initConversationStorage();
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            ...
            const sessions = (request.result as SessionData[]).sort((a, b) => b.timestamp - a.timestamp);
            resolve(sessions);
        };
    });
}
...
ipcRenderer.on('save-conversation-turn', async (event, data: ConversationTurnData) => {
    try {
...
function handleShortcut(shortcutKey: string): void {
...
const cheatingDaddyApp = document.querySelector('cheating-daddy-app') as CheatingDaddyAppElement;
